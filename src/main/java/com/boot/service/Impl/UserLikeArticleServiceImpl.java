...
import com.boot.utils.FastjsonUtil;
...
public class UserLikeArticleServiceImpl implements UserLikeArticleService {
    Logger logger = LoggerFactory.getLogger(UserLikeArticleServiceImpl.class);
    /**
     * 文章点赞总数key
     * redis key命名规范推荐使用大写，单词与单词之间使用:在pro配置里有映射
     */
    ...
    public synchronized Long countUserLike(Long likedUserId) {
        validateParam(likedUserId);
            String o = (String) redisTemplate.opsForHash().get(TOTAL_LIKE_COUNT_KEY, String.valueOf(likedUserId));
            if(o!=null){
